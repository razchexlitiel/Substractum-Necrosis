package razchexlitiel.substractum.client.gecko.bullets;
...
public class TurretBulletModel extends GeoModel<TurretBulletEntity> {
    ...
    public ResourceLocation getModelResource(TurretBulletEntity object) {
        return new ResourceLocation(SubstractumMod.MOD_ID, "geo/turret_bullet.geo.json");
    }
    ...
    public ResourceLocation getTextureResource(TurretBulletEntity object) {
        ...
        if (path.startsWith(itemPrefix)) {
            ...
            if (suffix.isEmpty()) {
                return new ResourceLocation(SubstractumMod.MOD_ID, "textures/entity/" + bulletPrefix + ".png");
            }

            // Иначе это какой-то тип (fire, piercing, explosive)
            // Возвращаем "textures/entity/turret_bullet_fire.png"
            return new ResourceLocation(SubstractumMod.MOD_ID, "textures/entity/" + bulletPrefix + suffix + ".png");
        }

        // Фолбэк (если ID предмета совсем другой)
        return new ResourceLocation(SubstractumMod.MOD_ID, "textures/entity/turret_bullet.png");
    }

    @Override
    public ResourceLocation getAnimationResource(TurretBulletEntity animatable) {
        return new ResourceLocation(SubstractumMod.MOD_ID, "animations/turret_bullet.animation.json");
    }
}
